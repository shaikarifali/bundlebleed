...
async def capture_page(
    ...
    return RuntimeCapture(page_url=page_url, events=events, discovered_js_urls=discovered_js)
...
async def capture_runtime(
    ...
    for url in page_urls[:max_pages]:
        captures.append(
            await capture_page(url, guard, timeout_seconds=timeout_seconds, on_allowed=on_allowed)
        )
    return captures
